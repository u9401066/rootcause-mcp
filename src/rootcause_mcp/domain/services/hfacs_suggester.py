...
class HFACSSuggester:
            ...
            if "subcategories" in level_data:
                for subcat_id, subcat_data in level_data["subcategories"].items():
                    if isinstance(subcat_data, dict) and "codes" in subcat_data:
                        ...
                        extract_from_codes(subcat_data["codes"], level_num, subcat_name)
    ...
    def suggest(
        ...
        desc_normalized = (
            description if self.config.case_sensitive else description.lower()
        )
        ...
        for rule in self.rules:
            keyword = (
                rule.keyword if self.config.case_sensitive else rule.keyword.lower()
            )
        ...
        for suggestion in suggestions:
            if (
                suggestion.code.code not in seen_codes
                and float(suggestion.confidence) >= self.config.min_confidence
            ):
    ...
    def get_loaded_rules_summary(self) -> dict[str, Any]:
        ...
        for rule in self.rules:
            # By source
            summary["by_source"][rule.source] = (
                summary["by_source"].get(rule.source, 0) + 1
            )

            # By domain
            if rule.domain:
                summary["by_domain"][rule.domain] = (
                    summary["by_domain"].get(rule.domain, 0) + 1
                )
